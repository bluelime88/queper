...
// name is optional — when present it's prefixed to the body ("Jane, ...").
export function readyMessage(type, label, number, name) {
  const msg = base(type, label, number);
  if (name) msg.body = `${name}, ${msg.body.charAt(0).toLowerCase()}${msg.body.slice(1)}`;
  return msg;
}

function base(type, label, number) {
  switch (type) {
    case 'restaurant':
