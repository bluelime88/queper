...
export async function POST(req) {
  const { token, queueNumber } = await req.json();
  ...
  if (!session) {
    ...
    const { data, error } = await supabaseAdmin.from('queue_sessions')
      .insert({ business_id: biz.id, queue_number: num, status: 'waiting', joined_at: now.toISOString(), expires_at: expires })
      .select().single();
    ...
    const { data } = await supabaseAdmin.from('queue_sessions')
      .update({ status: 'waiting', joined_at: now.toISOString(), expires_at: expires })
